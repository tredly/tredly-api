...
function checkPartition (options) {
    ...
    return function* () {
        ...
        var fullPath = self.body && self.body.location || '';
        ...
        if (options.checkContainer) {
            ...
            result &= _.some(containers.data, function (container) {
                var partitionName = container['Partition'];
                if (partitionName === '-') {
                    partitionName = 'default';
                }
                return container['UUID'] === containerId && self.user.partitions.indexOf(partitionName) >= 0;
            });
        }
